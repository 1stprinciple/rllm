...
about the update of self.sampler (currently update only once per batch).
"""
...
class FireworksBackend(BackendProtocol[Iterable, list[tinker.Datum]]):
    """
    Fireworks backend for the unified trainer.

    This backend provides:
        - Fireworks-specific rollout engine (FireworksEngine)
        - Policy training via FireworksPolicyTrainer
        - Checkpoint management via Fireworks' checkpoint utilities

    The backend uses async methods naturally to match Fireworks' async API.
    """

    name: str = "fireworks"
    requires_loop: bool = True  # Fireworks uses async operations
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize the FireworksBackend.
        ...
        # Fireworks service client
        self.service_client = FiretitanServiceClient(base_url=config.tinker_base_url)
    ...
    def validate_config(self) -> None:
        """Validate Fireworks-specific configuration settings."""
    ...
    async def generate_episodes(
        ...
        **kwargs,
        ...
        assert self.sampler is not None, "sampling_client is not initialized"

        # Set the sampling client on the rollout engine
        self.rollout_engine.set_sampling_client(self.sampler)
    ...
    async def on_train_start(self, trainer_state: TrainerState) -> None:
        ...
        start_batch, self.sampler = await self.policy_trainer.initialize_async(resume_from_checkpoint=True)
    ...
    async def on_batch_end(self, trainer_state: TrainerState) -> None:
        ...
        with simple_timer("save_checkpoint", trainer_state.timing_dict):
            ...
            self.sampler = await self.policy_trainer.save_checkpoint_and_get_sampling_client(global_step, kind="both", do_save=do_save)
